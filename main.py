...
def set_student_fees():
    ...
    with st.expander("👀 View All Student Fees"):
        fees_data = load_student_fees()
        if not fees_data:
            st.info("No student fee settings found")
        else:
            fee_records = [
                {
                    "Student ID": student_id,
                    "Student Name": details["student_name"],
                    "Class Category": details["class_category"],
            ...
            if student_id:
                ...
                if not student_records.empty:
                    ...
                    display_df = student_records[[
                        "Month", "Monthly Fee", "Annual Charges", "Admission Fee", 
                        "Received Amount", "Payment Method", "Date", "Academic Year"
                    ...
                    st.markdown("---")
                    st.markdown(f"**Annual Charges Paid:** {'✅ Yes' if annual_paid else '❌ No'}")
        ...
        if not st.session_state.is_admin:
            ...
            if menu == "View All Records":
                ...
                if df.empty:
                    ...
                    merged = pd.merge(all_combinations, payment_records, 
                                    on=["ID", "Month"], how="left")
                    ...
                    student_summary.columns = [
                        ...
                        "Unpaid Months", "Total Outstanding"
                    ]
